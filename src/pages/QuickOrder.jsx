...
function QuickOrder() {
  ...
  const [painting, setPainting] = useState(() => {
    if (location.state?.painting) return location.state.painting
    const saved = sessionStorage.getItem('artlor_pending_quick_order')
    if (saved) {
      try {
        const parsed = JSON.parse(saved)
        return parsed.painting || null
      } catch (e) {
        return null
      }
    }
  ...
  useEffect(() => {
    async function initAuth() {
      try {
        ...
        const pendingKey = 'artlor_pending_quick_order'
        ...
        if (currentSession) {
          if (saved) {
            try {
              ...
              if (parsed.painting) {
                setPainting(parsed.painting)
              }
            } catch (e) {
              console.error('Failed to parse saved quick order:', e)
            } finally {
  ...
  const handleGoogleVerify = async () => {
    setAuthLoading(true)
    try {
      sessionStorage.setItem('artlor_pending_quick_order', JSON.stringify({ form, step, painting }))
  ...
  const canProceed = () => {
    ...
    if (step === 4) return session ? isValidAuthenticEmail(form.email).isValid : true
    return false
  }
  ...
  const submitQuickOrder = async () => {
    ...
    try {
      ...
      // Map style to expected backend category enum
      let category = 'Landscape'
      if (painting && painting.style) {
        const style = painting.style.toLowerCase()
      ...
      // Dispatch matching request to Express/Socket.io backend
      ...
      try {
        const orderRes = await fetch('http://localhost:4000/api/orders', {
          ...
          body: JSON.stringify({
            ...
            category,
            description: `Reproduce gallery painting: "${painting?.title || 'Selected art'}" (Size: ${form.artworkSize === 'Custom Size' ? form.customSize : form.artworkSize})`,
        ...
        console.warn('[QuickOrder] Local matching server offline:', err.message)
      }
